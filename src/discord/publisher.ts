...
export class DiscordPublisher {
  ...
  async send(threadId: string, content: string): Promise<{ id: string }> {
    ...
    if (!channel?.isSendable()) throw new Error(`Discord thread ${threadId} is not sendable`);
    return channel.send(redact(content));
  }

  async edit(threadId: string, messageId: string, content: string): Promise<void> {
    const channel = await this.client.channels.fetch(threadId);
    if (!channel?.isTextBased()) throw new Error(`Discord thread ${threadId} is not text based`);
