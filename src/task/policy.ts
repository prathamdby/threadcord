...
export function validateTaskPolicy(request: TaskRequest, config: AppConfig): PolicyResult {
  ...
  if (request.pushOverride && !isExplicitPushBranchAllowed(request.pushOverride, request.branch)) {
    return { ok: false, reason: `Push override ${request.pushOverride} is not allowed for branch ${request.branch}.` };
  }
  return { ok: true };
}

export function targetBranchForTask(taskId: string, request: TaskRequest): string {
...
function isExplicitPushBranchAllowed(push: string, baseBranch: string): boolean {
