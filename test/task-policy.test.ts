...
import { assertRepoAllowed, matchesAny, targetBranchForTask, validateTaskPolicy } from '../src/task/policy.js';
...
describe('validateTaskPolicy', () => {
  ...
  it('rejects repos outside the allowlist', () => {
    const result = validateTaskPolicy({ ...baseRequest, repo: 'other/web' }, config);

    expect(result).toEqual({ ok: false, reason: 'Repo other/web is not allowed.' });
  });

  it('rejects models outside the allowlist', () => {
    const result = validateTaskPolicy({ ...baseRequest, model: 'openai/gpt-5' }, config);

    expect(result).toEqual({ ok: false, reason: 'Model openai/gpt-5 is not allowed.' });
  });

  it('allows pushing back to the task base branch', () => {
    const result = validateTaskPolicy({ ...baseRequest, pushOverride: 'main' }, config);

    expect(result).toEqual({ ok: true });
  });

  it('allows pushing to an agent branch namespace', () => {
    const result = validateTaskPolicy({ ...baseRequest, pushOverride: 'agent/task-1' }, config);

    expect(result).toEqual({ ok: true });
  });

  it('rejects push overrides to arbitrary branches', () => {
    const result = validateTaskPolicy({ ...baseRequest, pushOverride: 'production' }, config);
...
describe('targetBranchForTask', () => {
  ...
  it('honors an explicit push override', () => {
    expect(targetBranchForTask('task-abc', { ...baseRequest, pushOverride: 'main' })).toBe('main');
  });
});

describe('assertRepoAllowed', () => {
  it('rejects repos outside the allowlist', () => {
    expect(() => assertRepoAllowed('other/web', config.allowedRepos)).toThrow(/not allowed/);
  });

  it('accepts repos under an owner glob', () => {
    expect(() => assertRepoAllowed('acme/web', config.allowedRepos)).not.toThrow();
  });
});
